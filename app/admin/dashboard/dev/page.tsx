...
export default function DevAdminPage() {
  ...
  const fetchDocs = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/admin/page-meta?type=dev')
  ...
  async function saveTags() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/page-meta', {
        ...
        body: JSON.stringify({ page_type: 'dev', slug: editingDoc.slug, tags }),
      })
      if (!res.ok) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tighter">Dev Notes — Tags</h2>
        <p className="text-sm text-muted-foreground">
          Edit tags for dev docs. DB overrides replace HTML meta keywords on the public page.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-muted-foreground">No dev docs found in public/dev/.</p>
      ) : (
