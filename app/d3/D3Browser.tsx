...
import type { D3Group } from '@/lib/html-meta'

export function D3Browser({ groups }: { groups: D3Group[] }) {
  const [query, setQuery] = useState('')
  const [activeTag, setActiveTag] = useState<string | null>(null)

  const allTags = useMemo(() => {
    const set = new Set<string>()
    groups.forEach(g => g.docs.forEach(d => d.keywords.forEach(k => set.add(k))))
    ...
    return groups
      .map(g => ({
        ...
        docs: g.docs.filter(d => {
          ...
          const matchesTag = !activeTag || d.keywords.includes(activeTag)
    ...
    <>
      ...
      {filtered.length === 0 ? (
        <p className="text-muted-foreground text-sm">
          {query || activeTag ? 'No visualizations match your search.' : 'No visualizations yet.'}
        </p>
      ) : (
            ...
            <section key={g.folder}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {g.docs.map(doc => (
                  <a
                    key={doc.slug}
                    href={`/d3/${g.folder}/${doc.slug}.html`}
                    target="_blank"
